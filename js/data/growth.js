...
(function (G) {
  ...
  const AGE_ANCHORS = [0, 28, 90, 180, 270, 365, 540, 730, 1095, 1460, 1825, 2190, 2555, 2920, 3285, 3650, 4015, 4380, 4745, 5110, 5475, 5840, 6205, 6570];

  const WEIGHT_REF = {
    boy: {
      p3: [2.5, 3.4, 5.0, 6.4, 7.1, 7.7, 9.1, 10.2, 12.1, 13.7, 15.3, 17.2, 19.0, 21.2, 23.5, 26.0, 28.8, 31.8, 35.0, 40.5, 48.5, 52.0, 54.5, 56.0],
      p50: [3.3, 4.5, 6.4, 7.9, 8.9, 9.6, 10.9, 12.2, 14.3, 16.3, 18.3, 20.5, 22.9, 25.3, 28.1, 31.2, 34.4, 37.8, 45.0, 50.8, 56.0, 60.5, 63.0, 64.5],
      p97: [4.1, 5.6, 8.0, 9.7, 10.8, 11.5, 12.9, 14.5, 17.2, 19.4, 21.8, 24.4, 27.2, 30.3, 33.7, 37.5, 41.5, 45.7, 54.0, 60.5, 66.0, 72.0, 75.0, 77.0],
    },
    girl: {
      p3: [2.4, 3.2, 4.6, 5.8, 6.5, 7.0, 8.4, 9.5, 11.4, 13.1, 14.8, 16.7, 18.6, 20.7, 23.0, 25.6, 28.6, 32.0, 36.0, 38.0, 39.5, 40.5, 41.0, 41.5],
      p50: [3.2, 4.2, 5.8, 7.3, 8.2, 8.9, 10.2, 11.5, 13.9, 15.9, 17.9, 20.1, 22.4, 24.9, 27.7, 30.8, 34.3, 38.2, 43.5, 45.5, 47.0, 48.5, 49.5, 50.5],
      p97: [4.0, 5.2, 7.3, 8.9, 9.9, 10.6, 12.3, 13.9, 16.9, 19.1, 21.5, 23.9, 26.6, 29.6, 32.9, 36.6, 40.8, 45.2, 51.5, 54.0, 56.5, 58.0, 59.5, 60.5],
    },
  };
  const LENGTH_REF = {
    boy: {
      p3: [46.5, 51.0, 57.5, 63.0, 67.0, 70.5, 77.5, 82.1, 90.7, 96.7, 103.3, 109.1, 114.7, 120.2, 125.5, 130.7, 135.9, 141.2, 147.0, 152.5, 157.0, 162.0, 165.0, 167.0],
      p50: [50.5, 54.5, 61.4, 67.8, 72.0, 75.7, 82.3, 87.1, 96.1, 103.3, 110.0, 116.1, 121.8, 127.4, 132.9, 138.4, 143.9, 149.5, 156.2, 163.9, 169.6, 171.8, 173.6, 174.7],
      p97: [54.0, 58.0, 65.3, 71.5, 76.0, 79.5, 87.1, 92.3, 101.7, 109.9, 116.7, 123.1, 128.9, 134.6, 140.3, 146.1, 151.9, 157.7, 164.0, 171.0, 176.0, 178.5, 181.0, 182.5],
    },
    girl: {
      p3: [46.0, 50.5, 56.5, 62.0, 65.8, 69.0, 76.0, 80.6, 89.4, 95.8, 102.3, 108.3, 113.8, 119.3, 124.8, 130.3, 136.0, 141.9, 147.0, 149.5, 151.0, 152.0, 152.5, 153.0],
      p50: [49.8, 53.5, 59.9, 66.0, 70.1, 74.0, 80.7, 85.7, 95.1, 102.3, 109.1, 115.1, 120.7, 126.3, 131.9, 137.5, 143.3, 149.3, 153.5, 155.5, 156.5, 157.5, 158.0, 158.2],
      p97: [53.0, 57.0, 64.0, 70.0, 74.5, 78.0, 85.7, 91.1, 100.7, 108.9, 116.0, 122.3, 128.2, 133.9, 139.6, 145.4, 151.4, 157.5, 161.5, 163.0, 164.0, 164.5, 165.0, 165.2],
    },
  };
  ...
  G.growth = {
    weightPercentile(state) {
      const age = clamp(state.ageDays, 0, 6570);
      return percentileOf(
        state.child.weight,
    ...
    applyGrowth(state, days) {
      ...
      else if (c.gender === 'boy' && age <= 5475) dailyWeight = 16 + randInt(-5, 6); // 青春期猛长：男孩三年约+16kg
      else if (c.gender === 'girl' && age <= 5475) dailyWeight = 5 + randInt(-3, 4);
      else dailyWeight = c.gender === 'boy' ? 8 + randInt(-4, 5) : 3 + randInt(-3, 3); // 15-18岁：尾声增重
      ...
      const dailyLen = age <= 28 ? 0.12 : age <= 90 ? 0.112 : age <= 180 ? 0.071 : age <= 270 ? 0.047
        ...
        : age <= 5475 ? (c.gender === 'boy' ? 0.0183 : 0.0065) // 青春期冲刺：男孩三年+20cm（13-14岁最猛），女孩+7cm基本定型
        : (c.gender === 'boy' ? 0.0049 : 0.0016); // 15-18岁：男孩再+5cm，女孩+1.7cm定型
